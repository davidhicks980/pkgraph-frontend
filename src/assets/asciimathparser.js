export default class AsciiMathParser {
  ...
  error(message, pos) {
    ...
    throw new Error(
      `Error at character ${pos} near "${neighbourhood}": ${message}`
    );
  }
  ...
  simple(pos = 0) {
    return this.longest([
      this.matrix(pos),
      this.bracketed_expression(pos),
      this.binary(pos),
      this.constant(pos),
      this.text(pos),
      this.unary(pos),
      this.negative_simple(pos),
    ]);
  }
  ...
  matrix(pos = 0) {
    ...
    const right = lr
      ? this.leftright_bracket(contents.end, "right")
      : this.right_bracket(contents.end);
    if (!right) {
      return;
    }
    const contents_tex = contents.rows.map((r) => r.tex).join(" \\\\ ");
    const matrix_tex = contents.is_array
      ? `\\begin{array}{${contents.column_desc}} ${contents_tex} \\end{array}`
      : `\\begin{matrix} ${contents_tex} \\end{matrix}`;
    return {
  ...
  matrix_contents(pos = 0, leftright = false) {
    ...
    while (
      !this.eof(end) &&
      !(leftright ? this.leftright_bracket(end) : this.right_bracket(end))
    ) {
      if (rows.length) {
        const comma = this.exact(",", end);
        if (!comma) {
  ...
  bracketed_expression(pos = 0) {
    ...
    if (l) {
      ...
      if (middle) {
        const r =
          this.right_bracket(middle.end) ||
          this.leftright_bracket(middle.end, "right");
        if (r) {
          return {
        ...
        const r =
          this.right_bracket(l.end) || this.leftright_bracket(l.end, "right");
        if (r) {
          return {
  ...
  text(pos = 0) {
    ...
    const textfn = this.match(
      /^(?:mbox|text)\s*(\([^)]*\)?|\{[^}]*\}?|\[[^\]]*\]?)/,
      pos
    );
    if (textfn) {
      const text = this.escape_text(
        textfn.match[1].slice(1, textfn.match[1].length - 1)
      );
      return {
        tex: `\\text{${text}}`,
        pos: pos,
  ...
  constant(pos = 0) {
    ...
    return this.longest([
      this.other_constant(pos),
      this.greek(pos),
      this.name(pos),
      this.number(pos),
      this.arbitrary_constant(pos),
    ]);
  }
  ...
  arbitrary_constant(pos = 0) {
    if (!this.eof(pos)) {
      ...
      for (let nc of this.non_constant_symbols.concat(
        this.left_brackets.map((x) => x.asciimath),
        this.right_brackets.map((x) => x.asciimath),
        this.leftright_brackets.map((x) => x.asciimath)
      )) {
